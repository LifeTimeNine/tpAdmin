define(function () {
    return function (element, InitHandler, UploadedHandler, CompleteHandler) {
        ...
        var progressFilter = $(element).data('progress-filter') || '';
        var uploadOptions = {
            proindex: 0,
            elem: element,
            auto: false,
                ...
                obj.preview(function(index, file) {
                    $.ajax({
                        ...
                        success: function (ret) {
                            ...
                            if (!progressFilter && showProgress) {
                                var progress = $('<div class="layui-progress margin-top-10 margin-bottom-10">')
                            ...
                            if (ret.data.uploadId) {
                                shard(ret.data,file, progressFilter ? progressFilter : 'progress-'+field)
                            } else {
                                uploader(ret.data,file, progressFilter ? progressFilter : 'progress-'+field);
                            }
        ...
        function uploader(uploadParam,file, progressFilter) {
            ...
            $.ajax({
                type: "post", url: uploadParam.url, headers: header, data: formData,
        ...
        function shard(uploadParam, file, progressFilter) {
            ...
            var shardUpload = function (url, method, contentType, header, data, partNumber) {
                if (error) return
                taskNumber+= (taskNumber==-1?2:1);
                completeSize[partNumber] = 0;
                $.ajax({
                    type: method, url: url, headers: header, data: data,
                    contentType: contentType, processData: false,
                    xhr: function() {
                        var xhr = $.ajaxSettings.xhr();
                        if (xhr.upload) xhr.upload.addEventListener('progress', function (e) {
                            completeSize[partNumber] = e.loaded;
                            layui.element.progress(progressFilter, Math.floor(arrSum(completeSize)/totalSize*100) + '%');
                        });
                        return xhr;
                    },
            ...
            var initUpload = function () {
                ...
                if (shardOptionsList.length >= 1) {
                    ...
                    if (options.method == 'post') {
                        data = new FormData();
                        for(item of options.body) {
                            data.append(item.key, item.value);
                        }
                        data.append(options.fileFieldName, file.slice(shardSize*(options.partNumber-1), shardSize*options.partNumber));
                        contentType = false;
                    } else {
                        data = file.slice(shardSize*(options.partNumber-1), shardSize*options.partNumber);
                        contentType = options.contentType;
                    }
                    shardUpload(options.url,
                        options.method,
                        contentType,
                        keyValToObj(options.header ? options.header : []),
                        data,
                        options.partNumber);
                }
        ...
        function keyValToObj(arr) {
            let obj = {};
            for(let item of arr) {
                if (['Content-Length', 'Host', 'Content-Type'].indexOf(item.key) != -1) continue;
